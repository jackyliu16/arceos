...
use arr_macro::arr;
use axstd::net::{IpAddr, SocketAddr, UdpSocket};
use core::fmt::Write;
...
fn main() {
    ...
    let (addr, port) = (IpAddr::from([192,168,88, 77]), 65002);
    let socket65002 = UdpSocket::bind((addr, port).into()).unwrap();
    let target_socket_addr = SocketAddr::new(IpAddr::from([192,168,88,7]), 50001);

    socket65002.send_to(buf1, target_socket_addr).unwrap();
